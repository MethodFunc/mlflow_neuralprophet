...
def hyperopt_fn(params, train_data, validation_data, args, n_forecasts=24, n_lags=24):
    model = NeuralProphet(**params, n_forecasts=n_forecasts, n_lags=n_lags)
    model.add_lagged_regressor(names=args.col_name, normalize=True)
...
def hyperopt_fit(train_data, validation_data, args):
    trials = Trials()
    ...
    train_fn = partial(hyperopt_fn, train_data=train_data, validation_data=validation_data, args=args)
